...
// Get all states from ebusd deamon
function startSync(cb) {
	//var objects = {};
	var objects = [];
    // send command i to get list of ebus modules
    telnetOut.send('i', function (err, result) {
        if (err) {
        ...
        if (result) 
        {
            ...
            lines.forEach(function(element) {
                if (element.search("loaded \"") > -1)
                {
                    // there is a csv file in
                    /* sample strings
                    ...
                    */
                    ...
                    {
                        var csvFileName = splittedString[2].substring(9, splittedString[2].length-1);
                        csvFiles.push(csvFileName);
                        adapter.log.info(csvFileName);
                    }
                }
            }, this);
            ...
            csvFiles.forEach(function(element) {
                // read out the files
                var ebusDir = adapter.config.ebusdDirectory;
                var csvFile = ebusDir + "/" + element;
				adapter.log.info("CSV File: "+csvFile);
                ...
                fs.createReadStream(csvFile)
				  ...
				  .on('data', function (data) {
				  	...
				  	if (data.name.length > 0 && data["# type (r[1-9];w;u)"].trim() === '#')
				  	{
				  		channels[csvFile] = "."+data.circuit+".";
				  	}
					// create all states 
					if (data.name.length > 0 && data["# type (r[1-9];w;u)"].indexOf("r") > -1)
					{
						var chan = '.';
						if (channels[csvFile] != '')
						{
							chan = channels[csvFile];
						}
						var id = adapter.namespace + chan + data.name;
						...
						ebusdObjects[id] = objects.shift();
						  	
						var obj = ebusdObjects[id];
					  	// Create objects
					  	adapter.getForeignObject(obj._id, function (err, oldObj) {
							if (err) adapter.log.error("asdf2: "+err);

							if (!oldObj) {
								adapter.log.debug('Create "' + obj._id + '"');
								adapter.setForeignObject(obj._id, obj, function (err) {
									if (err) adapter.log.error("asdf:" + err);
					...
					// Update all 1 Minute the values

				  })
				  .on('end', function () {
						// We are done
						adapter.log.info("End "+csvFile);
						//adapter.log.info(JSON.stringify(ebusdObjects));
					})
            ...
            // Read input line by line and parse out slaves hardware and used configs for commands (variables)
            if (objects) {
        	...
        	// if there is no result end connection wait 60 seconds and reconnect and try again
        	adapter.log.info("no result");
        }
    });
...
{
	if (ebusdObjects != null)
	{
		for (var id in ebusdObjects) {
			queue.push({command: 'read', id: ebusdObjects[id]._id});
			//Do your logic with the property here
		}
	} else {
		adapter.log.error("EbusObjects are null");
	}
	processQueue();
}
...
function readValue(id, cb) {
    telnetOut.send('r -f ' + ebusdObjects[id]["native"].name, function (err, result, errValue) {
        if (err) 
        { 
        	adapter.log.error('readValue Error: ' + err);
        }
        if (!errValue && result) {
...
function executeReadAllValues(arg) {
  if (telnetOut != null && !telnetOut.isCommandRunning())
  {
	  readAllValues();
  }
  setTimeout(executeReadAllValues, 60000, 'readAll');
}
...
function main() {
    ...
    adapter.config.ebusdDirectory = adapter.config.ebusdDirectory || '/etc/ebusd';
	...
	telnetOut.on('error', function (error) {
    ...
    telnetOut.on('log', function(entry) {
    	adapter.log.info("Telnet: "+entry);
    });
	...
	setTimeout(executeReadAllValues, 5000, 'readAll');
